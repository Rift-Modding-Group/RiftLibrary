...
public class RawGeometryTree {
    ...
    public final RawModel.RawModelDescription description;

    public RawGeometryTree(RawModel model, ResourceLocation location) {
        RawModel.MinecraftGeometry geometry = model.geometry.getFirst();

        this.description = geometry.description;

        List<RawModel.RawModelBone> rawBones = new ArrayList<>(geometry.bones);
        int index = rawBones.size() - 1;
        int loopsWithoutChange = 0;
        while (true) {
            loopsWithoutChange++;
            if (loopsWithoutChange > 10000) {
                RiftLibraryMod.LOGGER.warn("Some rawBones in " + location.toString() + " do not have existing parents: ");
                RiftLibraryMod.LOGGER.warn(rawBones.stream().map(b -> b.name).collect(Collectors.joining(", ")));
                break;
            }

            RawModel.RawModelBone rawBone = rawBones.get(index);
    ...
    public boolean hasParent(RawModel.RawModelBone bone) {
