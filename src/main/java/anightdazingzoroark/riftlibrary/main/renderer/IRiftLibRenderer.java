...
import anightdazingzoroark.riftlibrary.main.geo.basic.*;
import anightdazingzoroark.riftlibrary.main.util.Color;
import anightdazingzoroark.riftlibrary.main.util.MatrixStack;
...
public interface IRiftLibRenderer<T> {
    MatrixStack MATRIX_STACK = new MatrixStack();

    default void render(RiftLibModel model, T animatable, float partialTicks, float red, float green, float blue, float alpha) {
        ...
        for (RiftLibBone group : model.topLevelBones) {
    ...
    default void renderRecursively(BufferBuilder builder, RiftLibBone bone, float red, float green, float blue, float alpha) {
        MATRIX_STACK.push();
        ...
        if (!bone.isHidden()) {
            for (RiftLibCube cube : bone.childCubes) {
                MATRIX_STACK.push();
    ...
    default void renderCube(BufferBuilder builder, RiftLibCube cube, float red, float green, float blue, float alpha) {
        MATRIX_STACK.moveToPivot(cube);
        MATRIX_STACK.rotate(cube);
        MATRIX_STACK.moveBackFromPivot(cube);

        for (RiftLibQuad quad : cube.quads) {
            ...
            for (RiftLibVertex vertex : quad.vertices) {
